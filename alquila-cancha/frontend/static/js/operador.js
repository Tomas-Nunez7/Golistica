...
async function viewCourtSchedule(courtId) {
    ...
    try {
        // Obtener horario de la cancha
        const response = await fetch(`/api/courts/${courtId}/schedule`);
        const data = await response.json();
        
        if (data.success) {
            showCourtScheduleModal(data.court, data.schedule);
        } else {
...
// Mostrar modal con horario de cancha
function showCourtScheduleModal(court, schedule) {
    ...
    schedule.forEach(booking => {
        if (!scheduleByDate[booking.date]) {
            scheduleByDate[booking.date] = [];
        }
        scheduleByDate[booking.date].push(booking);
    });
    
    // Generar HTML del horario
    let scheduleHTML = '';
    Object.keys(scheduleByDate).sort().forEach(date => {
        const bookings = scheduleByDate[date];
        scheduleHTML += `
            <div style="margin-bottom: 20px;">
                <h4 style="color: #333; margin-bottom: 10px;">${formatDate(date)}</h4>
        ...
        bookings.forEach(booking => {
            ...
            const paymentColor = booking.payment_status === 'paid' ? '#28a745' : '#ffc107';
    ...
    modal.innerHTML = `
        ...
        ">
            ...
            <h2 style="margin-bottom: 20px; color: #333;">
                <i class="fas fa-calendar"></i> Horario de ${court.name}
            </h2>
            
            <div style="margin-bottom: 20px; color: #666;">
                <strong>Precio:</strong> $${court.price}/hora
            </div>
            
            ${schedule.length > 0 ? scheduleHTML : 
                '<p style="text-align: center; color: #666; padding: 40px;">No hay reservas programadas para esta semana</p>'
            }
        </div>
    `;
